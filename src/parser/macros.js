...
export const parse_macro_def = (self) => {
	let {ok, err} = self.parse_inner("div", false, true)
	...
	self.macros[self.macros.length - 1][m_ok.type] = new Macro(m_ok.children, count_res.isVoid, m_ok.type, {file: self.path, col: self.col, line: self.ln})
	self.update_src()
